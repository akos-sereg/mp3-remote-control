...
public class Audio {
	...
	public static Line getMasterOutputLine() {
		String [] audioLines = Settings.AUDIO_LINE.split(",");
		
		for (Mixer mixer : getMixers()) {
			for (Line line : getAvailableOutputLines(mixer)) {
				System.err.println("LineInfo: " + line.getLineInfo().toString());
		
				for (String audioLine : audioLines)
				{
					if (line.getLineInfo().toString().contains(audioLine)) return line;	
				}
			}
		}
		return null;
